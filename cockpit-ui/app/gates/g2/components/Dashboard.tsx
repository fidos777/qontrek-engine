...
export default function Dashboard() {
  ...
  if (error) return <div className="p-6"><p className="text-[var(--text-danger)]" aria-live="polite">Error: {error}</p></div>;
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="p-4">
          <div className="text-sm text-[var(--text-muted)]">Total Recoverable</div>
          <div className="text-2xl font-bold">{fmMYR.format(Number(data.summary.total_recoverable || 0))}</div>
        </Card>
        <Card className="p-4">
          <div className="text-sm text-[var(--text-muted)]">7-Day Recovery Rate</div>
          <div className="text-2xl font-bold">{pct(kpi["recovery_rate_7d"])}</div>
          <div className="text-xs text-[var(--text-muted)] mt-2">Avg days to pay: {kpi["average_days_to_payment"] ?? "-"}</div>
        </Card>
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm text-[var(--text-muted)]">Pending Cases</div>
              <div className="text-xl font-semibold">{kpi["pending_cases"] ?? 0}</div>
            </div>
            <div>
              <div className="text-sm text-[var(--text-muted)]">Handover Queue</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card className="p-4">
          ...
          {data.critical_leads.length === 0 ? (
            <p className="text-sm text-[var(--text-muted)]">No critical overdue leads.</p>
          ) : (
        ...
        <div className="grid grid-cols-1 gap-4">
          <Card className="p-4">
            ...
            {data.active_reminders.length === 0 ? (
              <p className="text-sm text-[var(--text-muted)]">No active reminders scheduled.</p>
            ) : (
              <ul aria-label="Active reminders" className="space-y-2">
                {data.active_reminders.map((r: any, i: number) => (
                  <li key={i} className="flex items-center justify-between border border-[var(--border-default)] rounded px-3 py-2">
                    <div>
                      <div className="font-medium">{r.recipient ?? "-"}</div>
                      <div className="text-xs text-[var(--text-muted)]">{r.channel ?? "-"} · {r.scheduled_at ? fmDT.format(new Date(r.scheduled_at)) : "-"}</div>
                    </div>
                    <span className="text-xs px-2 py-1 rounded bg-[var(--bg-muted)]">{r.status ?? "-"}</span>
                  </li>
                ))}
              </ul>
            )}
          ...
          <Card className="p-4">
            ...
            {data.recent_success.length === 0 ? (
              <p className="text-sm text-[var(--text-muted)]">No recent payments.</p>
            ) : (
                ...
                {data.recent_success.map((r: any, i: number) => (
                  <li key={i} className="flex items-center justify-between border border-[var(--border-default)] rounded px-3 py-2">
                    <div>
                      ...
                      <div className="text-xs text-[var(--text-muted)]">Paid {r.paid_at ? fmDT.format(new Date(r.paid_at)) : "-"} · {typeof r.days_to_pay === "number" ? `${r.days_to_pay} days` : "-"}</div>
                    </div>
